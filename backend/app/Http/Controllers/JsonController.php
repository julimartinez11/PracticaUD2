...
class JsonController extends Controller
{
    ...
    {
        $ficheros = Storage::files('app');
        $ficherosJSON = [];

        foreach ($ficheros as $fichero) {
            if (pathinfo($fichero, PATHINFO_EXTENSION) === 'json') {
                $contenido = Storage::get($fichero);

                if ($this->isValidJson($contenido)) {
                    $ficherosJSON[] = basename($fichero);
                }
            }
        }
   ...
   /**
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (is_array($content)) {
            $content = json_encode($content); // Convertir array a cadena JSON si viene como array
        }
